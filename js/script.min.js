...
google.maps.event.addDomListener(window,"load",initialize);function changeValue(elementName,newValue){document.getElementsByName(elementName)[0].value=newValue;};$(document).ready(function(){$('form').submit(function(){if($('input[type="text"]#e-mail').val().length>0){$('form').attr('action','/');return false;}});});const demoUrl=['https://forms.gle/RdNoQYseQ8DVpTu66','https://share.hsforms.com/1mqJbPiylRJuIP6VMo-n1DQ5jwqm']
function clickBookDemo(){const randomIndex=Math.floor(Math.random()*2);const bookDemoLink=demoUrl[randomIndex];window.open(bookDemoLink);};$(document).ready(function(){const $productsList=$('.product-list');$('[id^=bookDemo_]').on('click',clickBookDemo);$('.nav-products').on('click',function(){$productsList.toggleClass('show');});$(document).on('click',function(event){const $target=$(event.target)
if($productsList.hasClass('show')&&!($target.is('[class*=product-list-item]')||$target.is('[class*=nav-products]')))$productsList.removeClass('show')})});